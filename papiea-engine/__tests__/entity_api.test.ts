...
import { processSortQuery } from "../src/utils/utils";
...
describe("Sorting tests", () => {
    ...
    test("Sorting with no explicit order should be ascending", async () => {
        expect.hasAssertions();
        try {
    ...
    test("Sorting with descending order", async () => {
        expect.assertions(1);
        try {
            const { data } = await entityApi.post(`${providerPrefix}/${providerVersion}/${kind_name}/filter?sort=spec.x:desc`, {
                spec: {
                    y: 11
                }
            });
            expect(data.results[0].spec.x).toBe(69);
        } catch (e) {
            throw e;
        }
    });

    test("Sorting with wrong order parameter", async () => {
        expect.assertions(1);
        try {
            await entityApi.post(`${providerPrefix}/${providerVersion}/${kind_name}/filter?sort=spec.x:down`, {
                spec: {
                    y: 11
                }
            });
        } catch (e) {
            expect(e.response.data.errors[0]).toBe("Sorting key's value must be either 'asc' or 'desc'");
        }
    });

    test("Sorting with multiple params", async () => {
        expect.assertions(1);
        try {
            const { data } = await entityApi.post(`${providerPrefix}/${providerVersion}/${kind_name}/filter?sort=spec.y,spec.x`, {
                spec: {
                    y: 11
                }
            });
    ...
    test("Query transformation function returns valid dict", async () => {
        await fc.assert(fc.property(fc.base64String(), fc.oneof(fc.constant("asc"), fc.constant("desc"), fc.string()), (prop, ord) => {
            ...
            if (errorExpected) {
                try {
                    processSortQuery(`${prop}:${ord}`)
                } catch (e) {
                    expect(e.errors[0]).toBe("Sorting key's value must be either 'asc' or 'desc'")
                }
            } else {
                expect(processSortQuery(`${prop}:${ord}`)).toEqual(expected)
            }
        }))
    })
