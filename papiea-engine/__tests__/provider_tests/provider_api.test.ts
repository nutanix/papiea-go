...
describe("Provider API tests", () => {
    ...
    test("Kinds with nullable fields should not get registered", async () => {
        ...
        try {
            ...
            expect(e.response.data.error.error_details.message).toContain("Papiea doesn't support 'nullable' fields. Please make a field 'host' non-required instead.")
        }
    });
    ...
    test("Nullable procedure fields shouldn't be accepted", async () => {
        ...
        try {
            ...
            expect(e.response.status).toEqual(400)
            expect(e.response.data.error.error_details.message).toContain("Papiea doesn't support 'nullable' fields. Please make a field 'x' non-required instead.")
        }
    });
